...
CSV_FILE="mockData_duplicates.csv"
...
with MongoClient(URI_STRING, auto_encryption_opts=csfle_opts) as client:
    with open(CSV_FILE, 'r') as csvfile:
        ...
        for row in spamreader:
            ...
            # print(doc)
            found = False
            # check if we have the same person id with the same address_type
            for x in client.csfle_demo.people.find({"PERSONID": row[0], "OtherInfo.ADDRESS_TYPE": row[11]},  {"PERSONID":1,"_id":0}):
                found = True
                print("Found matching personID with same address type", x)
            if not(found):
                print("No matching personID found, inserting...")
